...
LargeMatrixFinal <- read_delim("LargeMatrixFinal.csv", 
...
LargeMatrixFinal$iphonesentiment <- as.factor(LargeMatrixFinal$iphonesentiment)
...
write.csv(Errorplot, file = "KNNErrorplotIPHONE.csv")


# Make sentiment predictions on Large matrix ------------------------------

FinalPrediction <- predict(object = RFfit, newdata = LargeMatrixFinal)

FinalPrediction

Specialtable2 <- cbind(LargeMatrixFinal, FinalPrediction )

summary(FinalPrediction)

write.csv(Specialtable2, file = "FinalPredictionsRFiPhone.csv")
